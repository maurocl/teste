...
public class DummyActivity3 extends Activity implements Constantes {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		if (imagem == null) {
			Log.w(TAG,
					"onCreate() - Não foi possível obter uma instância da classe ManipulaImagm");
		}
		...
		if (mPreferences == null) {
			...
			Log.w(TAG,
					"mPreferences is null. Falha na execução do comandos getSharedPreferences()");
			// molduraPolaroid = PATH_MOLDURAS +
		...
		if ((molduraPolaroid != null) && (molduraPolaroid.equals(""))) {
			Log.d(TAG, "Moldura formato Polaroid não foi configurada");
			estadoFinal();
		}

		if ((molduraCabine != null) && (molduraCabine.equals(""))) {

			Log.d(TAG, "Moldura formato Cabine não foi configurada");
			estadoFinal();
		}

		// o arquivo contendo a moldura é lido no inicio da activity
		Log.d(TAG, "Lendo arquivo contendo moldura formato Polaroid");
		mBitmapMolduraPolaroid = leArquivoMoldura(molduraPolaroid);
		Log.d(TAG, "Lendo arquivo contendo moldura formato Cabine");
		mBitmapMolduraCabine = leArquivoMoldura(molduraCabine);

		Log.d(TAG, "onCreate() - mBitmapMolduraPolaroid =" + mBitmapMolduraPolaroid);
		Log.d(TAG, "onCreate() - mBitmapMolduraCabine = " + mBitmapMolduraCabine);

		// obtem o nº da câmera usada para tirar as fotos

		String s = getSharedPreference("pref_email",
				"preferencias_num_camera_frontal");
		...
		if ((s != null) && (!s.equals(""))) {
			...
			mCurrentCamera = 0;
			Log.w(TAG,
					"onCreate() - Nº da câmera frontal não foi definido. Assumindo o valor 0");
		}
		...
		btn0.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				...
				if (b == false) {
					Log.w(TAG,
							"onCreate() - Não foi possível inicializar as variáveis");
					estadoFinal();
				}
	...
	private void iniciaProcesso() {
		...
		if (b == false) {
			Log.w(TAG,
					"iniciaProcesso() - Não foi possível inicializar as variáveis");
			estadoFinal();
		}
	...
	protected void onActivityResult(int requestCode, int resultCode, Intent data) {
		...
		Log.i(TAG, "onActivityResult(request " + requestCode + ", result="
		...
		if (requestCode == ACTIVITY_PARTICIPANTE) {
			...
			Log.w(TAG, "onActivityResult() - Erro ... requestCode: "
					+ requestCode + " não pode ser processado");
	...
	private boolean iniciaVariaveis() {
		...
		Log.d(TAG,
				"iniciaVariaveis() - Número de Câmeras disponíveis no hardware: "
						+ numCameras);

		// verifica se a câmera fotogrática está em operação
		if (isCameraWorking(mCurrentCamera)) {

			Log.i(TAG, "iniciaVariaveis() - Camera: [" + mCurrentCamera
					+ "] está em funcionamento...");

		} else {

			Log.w(TAG, "iniciaVariaveis() - Camera: [" + mCurrentCamera
					+ "] não está em funcionamento");

			Toast.makeText(this, "Camera não está disponível",
					Toast.LENGTH_SHORT);
		...
		if (i.getSerializableExtra("br.com.mltech.contratante") != null) {
			mContratante = (Contratante) i
					.getSerializableExtra("br.com.mltech.contratante");
		} else {
			Log.w(TAG, "iniciaVariaveis() - Contratante não pode ser nulo.");
			Toast.makeText(this, "Contratante não pode ser nulo",
		...
		if (i.getSerializableExtra("br.com.mltech.evento") != null) {
			...
			Toast.makeText(this, "Evento não pode ser nulo", Toast.LENGTH_SHORT)
					.show();
			erro += 2;
		}
		...
		if (erro > 0) {
			...
			Log.w(TAG,
					"iniciaVariaveis() - Informações insuficientes para execução (erro="
							+ erro + ")");
	 ...
	 * startActivityParticipante()
	 * 
	 * Inicia a Activity Participante. Passa como parâmetro as informações sobre
	 * o Evento.
	 * 
	...
	private void resultActivityParticipante(int resultCode, Intent data) {
		...
		Log.d(TAG,
				"resultActivityParticipante() ==> processando resultado da ACTIVITY PARTICIPANTE");

		if (getEstado() < 0) {
			Log.w(TAG,
					"resultActivityParticipante() - Não há informações sobre o participante");
			return;
		}
		...
		if (resultCode == RESULT_CANCELED) {
			...
			// operação cancelada
			Log.d(TAG,
					"resultCode=RESULT_CANCELED - Participante cancelou sua participação");
			// Limpa as variáveis
			mParticipante = null;
			mParticipacao = null;
			return;
			...
			Log.w(TAG,
					"resultActivityParticipante() - resultCode não conhecido: "
							+ resultCode);
			return;
		...
		if (data == null) {
			...
			Log.w(TAG,
					"resultActivityParticipante() - A intent não retornou nenhuma informação");
			return;
		}
		...
		if (data.getSerializableExtra("br.com.mltech.participante") != null) {
			mParticipante = (Participante) data
					.getSerializableExtra("br.com.mltech.participante");
		}

		if (data.getSerializableExtra("br.com.mltech.participacao") != null) {
			mParticipacao = (Participacao) data
					.getSerializableExtra("br.com.mltech.participacao");
		}
	...
	private void obtemFoto() {
		...
		if (getEstado() < 1) {
			Log.d(TAG,
					"obtemFoto() - não há informações sobre o participante. Processo abortado !");
			estadoFinal();
		}
		...
		if (isCameraWorking(mCurrentCamera)) {
			...
			Log.w(TAG,
					"obtemFoto() - Erro da obtenção da instância da Camera. Processo abortado !");
			estadoFinal();
		}
		...
		Log.w(TAG,
				"obtemFoto() - f.getAbsolutePath()=" + file.getAbsolutePath());
		...
		if (file.canWrite()) {
			Log.d(TAG, "obtemFoto() - arquivo: " + mFilename
					+ " pode ser gravado");
		} else {
			Log.w(TAG, "obtemFoto() - arquivo: " + mFilename
		...
		if (xUri == null) {
			Log.w(TAG,
					"obtemFoto() - xUri=null. Arquivo para armazenamento não foi criado.");
			estadoFinal();
		} else {
			Log.i(TAG, "obtemFoto() ===> xUri=" + xUri.getPath() + ", xUri="
					+ xUri);
		}
	 ...
	 * 
	 * Se a activity executou com sucesso então a foto encontra-se no caminho
	 * dado por xUri.getPath(); mFilename possui o nome do arquivo gerado
	 * 
	 * @param resultCode
	...
	private void resultActivityTiraFoto3(int resultCode, Intent data) {
		...
		Log.d(TAG,
				"resultActivityTiraFoto3() ==> Executando o resultado do processamento da ACTIVITY_TiraFoto3");
		...
		if (resultCode == RESULT_OK) {
			...
			if (xUri == null) {
				// Uri is null
				Log.d(TAG,
						"resultActivityTiraFoto3() - xUri is null ==> nenhuma foto foi obtida");
				estadoFinal();
			}
			...
			// sistema
			Log.d(TAG,
					"resultActivityTiraFoto3() - Foto tirada e armazenada em xUri.getPath()="
							+ xUri.getPath());
			...
			Log.d(TAG, "resultActivityTiraFoto3() - mParticipacao="
					+ mParticipacao);
			...
			Log.w(TAG,
					"resultActivityTiraFoto3() - Operação tirar foto cancelada pelo usuário");
	...
	private void processaFotos() {
		...
		if (mParticipacao == null) {
			//
			Log.d(TAG,
					"processaFotos() - não é possível obter as informações do participante");
			estadoFinal();
		}

		if (mEvento == null) {
			Log.d(TAG,
					"processaFotos() - não é possível obter as informações sobre o evento");
			estadoFinal();
		}
	...
	private String processaTipoFoto(int tipoFoto, String pathCabine) {
		...
		if (tipoFoto == TIPO_FOTO_POLAROID) {
			...
			Log.w(TAG, "processaFotos() - tipo de foto: " + tipoFoto
					+ " não suportado.");
			estadoFinal();
	...
	private Bitmap processoEfeitoFiltroFoto(int efeitoFoto, Bitmap bitmap) {
		...
		if (efeitoFoto == CORES) { // aplica efeito cores COR
			...
			Log.w(TAG, "Efeito: " + efeitoFoto
					+ " não é suportado pela aplicação");
		}
	...
	private String formataFotoPolaroid(Bitmap bmMoldura) {
		...
		if (gravou) {
			...
			Log.i(TAG, "formataFotoPolaroid() - (Foto9x12): " + arqSaida
					+ " gravado com sucesso.");
		} else {
			// falha na gravação da foto
			Log.i(TAG,
					"formataFotoPolaroid() - (Foto9x12) - falha na gravação do arquivo: "
							+ arqSaida);
		}
		...
		if (gravou2) {
			// foto armazenada com sucesso
			Log.i(TAG, "formataFotoPolaroid() - (Foto8x8):" + arqSaida
					+ " gravado com sucesso.");
		} else {
			// falha na gravação da foto
			Log.i(TAG,
					"formataFotoPolaroid() - (Foto8x8) - falha na gravação do arquivo: "
							+ arqSaida);
		}
	...
	String gravaFotoPolaroidComMoldura(Bitmap bmFoto, Bitmap bmMoldura) {
		...
		if (bmFoto != null) {
			Log.w(TAG,
					"gravaFotoPolaroidComMoldura() - bmFoto is null");
		}

		if (bmMoldura != null) {
			Log.w(TAG,
					"gravaFotoPolaroidComMoldura() - bmMoldura is null");			
		}
		...
		if ((bmFoto != null) && (bmMoldura != null)) {
			...
			if (fotoComMoldura == null) {
				Log.w(TAG,
						"gravaFotoPolaroidComMoldura() - erro na conversão da foto");
			}
		...
		if (gravou) {
			// foto armazenada com sucesso
			Toast.makeText(this, "Foto gravada no arquivo: " + arqSaida,
					Toast.LENGTH_SHORT).show();
			Log.d(TAG,
					"gravaFotoPolaroidComMoldura() - Foto gravada no arquivo: "
							+ arqSaida);
		} else {
			// falha na gravação da foto
			Toast.makeText(this,
					"Falha na gravação da foto no arquivo: " + arqSaida,
					Toast.LENGTH_SHORT).show();
			Log.d(TAG,
					"gravaFotoPolaroidComMoldura() - Falha na gravação da foto no arquivo: "
							+ arqSaida);
		}
	 ...
	 * 
	 * foto formato Cabibe exige três fotos. as fotos serão montadas em
	 * sequencoa e será aplicada. uma moldura (conforme configuração). observe
	 * que a moldura está relacionada ao evento em andamento. portanto é
	 * necessário ter informações sobre o evento.
	 * 
	 * @param pathCabine
	 */
	private String formataFotoCabine(String pathCabine) {
		...
		if (pathCabine == null) {
			Log.d(TAG,
					"formataFotoCabine() - Não há moldura definida para foto CABINE");
		}
		...
		String nomeDoArquivo = imagem.processaFotoFormatoCabine(foto1, foto2,
				foto3, moldura);
	...
	private String formataFotoCabine2(Bitmap bmMoldura) {
		...
		if (bmMoldura == null) {
			Log.d(TAG,
					"formataFotoCabine2() - Não há moldura definida para foto CABINE");
		}
		...
		String nomeDoArquivo = imagem.processaFotoFormatoCabine2(foto1, foto2,
				foto3, bmMoldura);
	...
	Bitmap formataFoto3x4(Bitmap foto, String nome) {
		...
		// grava a foto redimensionada em um arquivo
		boolean gravou = imagem
				.gravaBitmapArquivo(fotoRedimensionada, arqSaida);
	...
	String xxxabc(Bitmap bmMoldura) {
		...
		if (gravou) {
			// foto armazenada com sucesso
			Toast.makeText(this, "Foto gravada com sucesso", Toast.LENGTH_SHORT)
					.show();
		} else {
			// falha na gravação da foto
			Toast.makeText(this, "Falha na gravação da foto",
					Toast.LENGTH_SHORT).show();
		}
	...
	private void enviaEmail(Uri lastUri) {
		...
		if (!erro) {
			...
			SharedPreferences emailPreferences = getSharedPreferences(
					"pref_email", MODE_PRIVATE);

			if (emailPreferences == null) {
				Log.w(TAG,
						"enviaEmail() - SharedPreferences não foi encontrada.");
			}

			/**
			 * Assunto do email
			 */
			String subject = emailPreferences.getString("preferencias_assunto",
					"Evento Inicial");

			/**
			 * Corpo do email
			 */
			String body = emailPreferences.getString("preferencias_descricao",
	 ...
	 * 
	 */
	private void sendEmail(String emailParticipante, String emailContratante,
			String subject, String text, Uri imageUri) {
		...
		Log.d(TAG, "sendEmail() - Anexando o arquivo: " + imageUri
				+ " aos anexos ...");
	...
	private void sendEmailByChooser(Intent emailIntent) {
		...
		Intent chooser = Intent.createChooser(emailIntent,
				"Selecione sua aplicação de email !");

		if (chooser != null) {

			Log.w(TAG,
					"sendEmailByChooser() - chooser.getAction()="
							+ chooser.getAction());
			...
			if (compName != null) {
				Log.w(TAG,
						"==> CHOOSER ==> compName=" + compName.getClassName()
								+ ", compName=" + compName.getPackageName());
			} else {
	...
	private void sendEmailRedesSociais() {
		...
		if (mEvento == null) {
			Log.d(TAG,
					"sendEmailRedesSociais() - Não foi possível obter os dados do evento.");
			return;
		}
	...
	private void resultActivityChooser(int resultCode, Intent data) {
		...
		Log.d(TAG,
				"resultActivityChooser() ===> processando resultado da ACTIVITY CHOOSER: resultCode="
						+ resultCode);
		...
		if (intent != null) {
			...
			if (compName != null) {
				Log.d(TAG, "compName=" + compName.getClassName()
						+ ", compName=" + compName.getPackageName());
			}
		}
		...
		if (data != null) {
			Log.w(TAG,
					"resultActivityChooser() - dados recebidos não foram trabalhados");
			// The action of this intent or null if none is specified.
			Log.w(TAG,
					"resultActivityChooser() - data.getAction()="
							+ data.getAction());
		}
		...
		if (resultCode == 0) {
			...
			Toast.makeText(this, "Email enviado com sucesso", Toast.LENGTH_LONG)
					.show();
			...
			Log.d(TAG,
					"resultActivityChooser() - ACTIVITY_CHOOSER - o envio de email foi cancelado");
			estadoFinal();

		} else {
			Log.d(TAG,
					"resultActivityChooser() - ACTIVITY_CHOOSER - resultCode "
							+ resultCode + " não pde ser tratado");
			estadoFinal();
		}
	...
	private void estadoFinal() {
		...
		if (estadoCorrente == 3) {
			...
			Toast.makeText(this,
					"Falha no processo. Estado atual: " + estadoCorrente,
					Toast.LENGTH_SHORT).show();

			// estado final atingido porém houve falha
			Log.w(TAG,
					"estadoFinal() - não foi possível chegar ao final do processamento.");
	...
	protected void onStop() {
		...
		/*
		 * Essa atribuição não deve ser feita pois perdemos o valor de xUri que
		 * será usado posteriormente mFilename = null; mUri = null; xUri = null;
		 */
	...
	public boolean isExternalMediaMounted() {
		...
		// externo
		isMounted = Environment.MEDIA_MOUNTED.equals(Environment
	 ...
	 * 
	 * Tenta obter uma instância da câmera identificada por seu número
	 * (cameraID). A primeira câmera, se existir terá o identificador 0.
	 * 
	 * @param cameraID
	...
	public static Camera getCameraInstance(int cameraID) {
		...
		try {
			...
			/*
			 * Cria um novo objeto Camera para acesso a câmera cameraID. Se a
			 * câmera já estiver em uso ou não existir retorma nulo
			 */
			...
			Log.w(TAG,
					">>> getCameraInstance("
							+ cameraID
							+ ") - Câmera está indisponível (está em uso ou não existe)",
					e);
	 ...
	 * 
	 * Verifica se a câmera identificada por cameraID está em funcionamento.
	 * Para fazer essa verificação é tentado abrir a câmera. Caso ela consiga
	 * ser aberta indica que a câmera está disponível e nesse caso simplesmente
	 * liberamos a câmera para uso. Se houver erro indica que há problemas;
	 * 
	 * @param cameraID
	 *            Identificador da câmera do dispositivo
	 * 
	 * @return true se for possível obter ima instância da classe Camera ou
	 *         false, caso contrário.
	 */
	boolean isCameraWorking(int cameraID) {
		...
		if (c != null) {
			...
			Log.i(TAG, "isCameraWorking() - câmera: " + cameraID
					+ " liberada com sucesso");

			return true;

		} else {
			Log.i(TAG, "isCameraWorking() - erro na liberação da câmera: "
					+ cameraID);
			return false;
	 ...
	 *            Nome do arquivo
	 * 
	 * @return um bitmap com o arquivo contendo a moldura ou null no caso de
	 *         algum problema
	 */
	private Bitmap leArquivoMoldura(String arquivoMoldura) {

		Log.d(TAG,
				"leArquivoMoldura() - arquivoMoldura: ["+arquivoMoldura+"].");
		
		File moldura = new File(arquivoMoldura);
		
		if(!moldura.exists()) {
			Log.w(TAG,
					"leArquivoMoldura() - arquivoMoldura: ["+arquivoMoldura+"] não existe.");
		...
		if (bmMoldura == null) {
			...
			Log.w(TAG,
					"leArquivoMoldura() - arquivo contento a moldura está vazio.");
			return null;

		} else {
			Log.v(TAG, "leArquivoMoldura() - largura x altura da moldura: "
					+ imagem.getStringBitmapSize(bmMoldura));
	 ...
	 * 
	 */
	private String getSharedPreference(String sharedPreferencesName,
			String attribute) {
