...
"where date_format(data,'%Y%m%d') between '$fmtData1' and '$fmtData2' order by data";
  ...
  /**
   *
   * Obtem um array de despesas por categoria entre duas datas
   *
   * @param string $data1
   * @param string $data2
   */
  public function listarByCategoriaBetweenData($idCategoria, $data1,$data2) {
    ...
    $cmd="SELECT id, date_format(data,'%d/%m/%Y'), descricao, valor, categoria FROM despesa g where date_format(data,'%Y%m%d') between '$fmtData1' and '$fmtData2' and categoria=$idCategoria order by data";
