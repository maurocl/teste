 ...
 * Note that currently AsyncTwitter is NOT compatible with Google App Engine as
 * it is maintaining threads internally.
 * 
...
public final class AsyncTwitterFactory implements java.io.Serializable {
	...
	static {
		SINGLETON = new AsyncTwitterImpl(ConfigurationContext.getInstance(),
				TwitterFactory.DEFAULT_AUTHORIZATION);
	}

	/**
	 * Creates an AsyncTwitterFactory with the root configuration, with no
	 * listener. AsyncTwitter instances will not perform callbacks when using
	 * this constructor.
	 */
	...
	/**
	 * Creates a AsyncTwitterFactory with the specified config tree, with given
	 * listener
	 * 
	 * @param configTreePath
	 *            the path
	...
	/**
	 * Returns an instance associated with the configuration bound to this
	 * factory.
	 * 
	 ...
	 * Unlike
	 * {@link AsyncTwitter#setOAuthAccessToken(twitter4j.auth.AccessToken)},
	...
	public AsyncTwitter getInstance(AccessToken accessToken) {
		...
		if (null == consumerKey && null == consumerSecret) {
			throw new IllegalStateException(
					"Consumer key and Consumer secret not supplied.");
		}
